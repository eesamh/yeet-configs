...
5. Take actions based on human input. For discussion-type comments, post a reply in the PR thread rather than making a code change. Any comment posted on behalf of the human MUST begin with a clear identity disclosure so readers don't mistake it for the human author — start the comment with a line like `_Posted by Farty Bobo on behalf of @<github-handle>._` (or equivalent), then the reply body.
...
8. Re-request review from the original commenters if they had requested changes or notify them via a PR comment that their feedback has been addressed. Any notification comment MUST also open with the same Farty Bobo identity disclosure described in step 5.
